...
| 项 | 值 |
|---|---|
| 主名 | Information Frontier |
| 英文辅助标识 | DEEPTECH FRONTIER |
| Wordmark 锁定 | 主名 18px 墨黑 + 辅助标识 11px 大写灰(字距放大);首页 hero 大标题用金属拉丝质感(`.metal-text`) |

> **关于配色与"蓝"**:主名已由旧名「深蓝前沿」改为 **Information Frontier**。原「深蓝」取「墨色与克制」之意、从不是背景主色;改名后这一克制基调不变 —— 底仍为暖纸白,蓝(冷青 `#00E5FF`)只作点睛,全页面积 < 5%。这条仍是整套设计的题眼。
> **hero 大标题金属质感**:首页主标题用 `.metal-text`(银灰拉丝渐变 + `background-clip:text`),近灰度、呼应 §8「拉丝铝」材质 DNA,不引入饱和色;`@supports` 不支持时回退墨黑。仅用于 hero 超大标题,导航/页脚小字仍用墨黑保证可读。
